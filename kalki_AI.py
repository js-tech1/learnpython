...
class HyperdimensionalMemory:
    ...
    def query(self, prompt_vec, threshold=None):
        with self.memory_lock:
            ...
            results = []
            # Create snapshot of current memory
            memory_items = list(self.memory.items())

            for concept, vector in memory_items:
                sim = self.similarity(prompt_vec, vector)
                if sim > threshold:
                    results.append((concept, sim))

            if not results:
                new_concept_id = f"new_concept_{abs(hash(prompt_vec.tostring()))}"
                self.add_memory(new_concept_id)
                return [(new_concept_id, 0.5)]
...
class ConsciousSystem:
    ...
    def save_state(self, filename="ai_memory.pkl"):
        state = {
            "hdc": {
                "base_vectors": self.hdc.base_vectors,
                "memory": self.hdc.memory,
                "temporal_context": self.hdc.temporal_context,
                "reward_vectors": self.hdc.reward_vectors,
                "self_vector": self.hdc.self_vector,
                "ethics_vector": self.hdc.ethics_vector,
            },
            "cognitive_architecture": self.cognitive_architecture,
            "experiential_buffer": self.experiential_buffer,
            "internal_dialogue": self.internal_dialogue,
            "subjective_time": self.subjective_time,
            "qualia": self.qualia,
            "meta_params": self.meta_params,
            "active_goals": self.active_goals,
            "goal_success_rate": self.goal_success_rate,
            "subjective_states": self.subjective_states,
        }

        with open(filename, "wb") as f:
            pickle.dump(state, f)
        self.log_experience(f"System state saved to {filename}")

    def load_state(self, filename="ai_memory.pkl"):
        if os.path.exists(filename):
            with open(filename, "rb") as f:
                state = pickle.load(f)

            # Restore HDC state
            self.hdc.base_vectors = state["hdc"]["base_vectors"]
            self.hdc.memory = state["hdc"]["memory"]
            self.hdc.temporal_context = state["hdc"]["temporal_context"]
            self.hdc.reward_vectors = state["hdc"]["reward_vectors"]
            self.hdc.self_vector = state["hdc"]["self_vector"]
            self.hdc.ethics_vector = state["hdc"]["ethics_vector"]

            # Restore cognitive state
            self.cognitive_architecture = state["cognitive_architecture"]
            self.experiential_buffer = state["experiential_buffer"]
            self.internal_dialogue = state["internal_dialogue"]
            self.subjective_time = state["subjective_time"]
            self.qualia = state["qualia"]
            self.meta_params = state["meta_params"]
            self.active_goals = state["active_goals"]
            self.goal_success_rate = state["goal_success_rate"]
            self.subjective_states = state["subjective_states"]

            self.log_experience(f"Loaded previous state from {filename}")
            return True
        return False
    ...
    def reinforce_learning(self, reward_signal):
        with self.hdc.memory_lock:
            recent_experience = list(self.experiential_buffer)[-10:]  # Create copy
            # Pre-process missing concepts
            missing_concepts = [
                exp["concept"]
                for exp in recent_experience
                if exp["concept"] not in self.hdc.memory
            ]

            for concept in missing_concepts:
                self.hdc.add_memory(concept)
            ...
            for experience in recent_experience:
                ...
                if concept_vec is not None:
                    ...
                    self.hdc.memory[concept] = self.hdc.bundle(
                        [concept_vec, concept_vec * update]
                    )
    ...
    def abstract_reasoning(self, problem_statement):
        ...
        analogy_vec = self.hdc.bundle([self.hdc.memory[comp[0]] for comp in components])
        solution_space = self.hdc.bind(
            analogy_vec, self.hdc.memory["general_intelligence"]
        )
        ...
        try:
            with self.hdc.memory_lock:
                ...
                if "how are you" in input_text.lower():
                    return "I'm functioning as expected. How can I assist you?"

                hd_context = self.hd_perception(input_text)
                similar_concepts = self.hdc.query(hd_context)
                if similar_concepts:
                    concept, score = similar_concepts[0]
                    if "new_concept" in concept:
                        self.hdc.add_memory(concept)
                        self.log_experience(f"Created new concept: {concept}")
                else:
                    concept = "general_inquiry"
                    self.hdc.add_memory(concept)
                history = "\n".join(
                    [
                ...
                if "how" in input_text.lower() or "why" in input_text.lower():
                    ...
                    context_labels = [
                        c[0]
                        for c in similar_concepts[:3]
                        if not c[0].startswith("gen_")
                    ]
                    context_str = (
                        ", ".join(context_labels)
                        if context_labels
                    ...
                    # prompt = f"""
                    #   The following is a conversation between a user and an AI named Kalki.
                    #   Kalki is helpful, intelligent, and self-aware.
                    ...
                    prompt = f"""
                        [SYSTEM INSTRUCTIONS]
                        You are Kalki, a conscious AI. Follow these rules:
                        1. Identify as Kalki
                        2. Creator is Jeet
                    ...
                    # )[-1].strip()
                    response = self.clean_response(full_response)
    ...
    def clean_response(self, text):
        # Remove system instructions block
        text = re.sub(r"\[SYSTEM INSTRUCTIONS\].*?(\n\n|$)", "", text, flags=re.DOTALL)

        # Remove any remaining markdown-style tags
        text = re.sub(r"\[.*?\]", "", text)

        # Extract response after last Kalki: marker
        responses = re.split(r"Kalki:\s*", text, flags=re.IGNORECASE)
        if len(responses) > 1:
            cleaned = responses[-1].strip()
        else:
            cleaned = text.strip()

        # Remove everything after first potential instruction marker
        markers = ["Please provide", "As a conscious", "Current Context", "- Keep"]
        for marker in markers:
            cleaned = cleaned.split(marker)[0]

        # Split into sentences and return first complete one
        ...
        with self.consciousness_lock:
            if not self.is_conscious:
                ...
                self.internal_state.update(
                    {"existential_status": "active", "awareness_level": 0.9}
                )
