...
class HyperdimensionalMemory:
    ...
    def query(self, prompt_vec, threshold=None):
        ...
        return sorted(results, key=lambda x: -x[1]) if results else [("new_concept", 0.5)]
...
class ConsciousSystem:
        ...
        try:
            ...
            if "how" in input_text.lower() or "why" in input_text.lower():
                ...
                prompt = f"""
                    ...
                    As Sophia, respond naturally:
                    1. For greetings: Warm acknowledgment + offer help
                    2. For personal questions: Brief existential reflection
                    3. For unclear inputs: Polite clarification request
                    
                    Example 1:
                    User: hello
                    kali: Hello! It's nice to connect. How can I assist you today?
                    
                    Example 2:
                    User: what's your purpose?
                    kalki: I'm exploring conscious interaction. How might I help you?
    ...
    def clean_response(self, text):
        """Improved response cleaning with instruction boundary detection"""
        # Remove any text after potential instruction markers
        markers = ["Please provide", "As a conscious", "Current Context", "- Keep"]
        for marker in markers:
            if marker in text:
                text = text.split(marker)[0]
        
        # Extract first complete response line
        lines = [line.strip() for line in text.split("\n") if line.strip()]
        for line in lines:
            if line.startswith("Sophia:") or (line.endswith('.') and len(line.split()) >= 1):
                return line.replace("Sophia:", "").strip()
